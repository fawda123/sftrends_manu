...
\end{inparaenum}
Each case study is built around hypotheses that results from \ac{wrtds} models are expected to support, both as a general description and as a means for additional testing with alternative methods. 
