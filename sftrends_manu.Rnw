...
obs_sm <- select(delt_dat, Site_Code, Location, Date, nh, no23, din) %>% 
...
The observed time series for the ten Delta stations had substantial variation in scale among the nitrogen analytes and differences in apparent seasonal trends (\cref{fig:obsdat}).  In general, long-term (inter-annual) trends were not easily observed from the raw data.  Total \ac{din} for most stations was dominated by nitrite/nitrate whereas ammonium was present at lower concentrations.  However, C3 had a majority of \ac{din} composed of ammonium and others had larger concentrations ammonium relative to nitrite/nitrate depending on the time of year (e.g., P8, D26).  By location, observed concentrations decreased on average from the upper Delta (C3, C10, MD10, P8), to the middle (D19, D26, D28), and to the Suisun stations(D4, D6, D7). 
