...
\author{
  {\bf {\normalsize Marcus W. Beck$^{1*}$, Thomas W. Jabusch$^2$, Philip R. Trowbridge$^3$, David B. Senn$^3$}}
  ...
  \\\\{\textit {\normalsize $^2$Sacramenta-San Joaquin Delta Conservancy}}
	\\{\textit {\normalsize 1450 Halyard Drive, Suite 6, West Sacramento, CA 95691}}
...
The Delta region drains a 200 thousand km$^2$ watershed into the \ac{sfe}, which is the largest estuary on the Pacific coast of North America.  The watershed provides water to over 25 million people and irrigation for 18 thousand km$^2$ of agricultural land.  Water enters the \ac{sfe} through the Sacramento and San Joaquin rivers that have a combined inflow of approximately 28 km$^3$ per year, with the Sacramento accounting for 84\% of inflow to the Delta.  The \ac{sfe} system includes the Delta and subembayments of San Francisco Bay (\cref{fig:delt_map}).  Water dynamics in the \ac{sfe} and Delta are governed by inflows from the watershed, tidal exchange with the Pacific Ocean, and water withdrawals for municipal and agricultural use \citep{Jassby00}.  Seasonally, inflows from the watershed peak in the spring and early summer from snowmelt.  Long-term trends have shown that consumption, withdrawals, and export have steadily increased from 1960 to present, although climate effects have contributed to inter-annual variability \citep{Cloern12b}. Notable drought periods have occurred from 1976-1977, 1987-1992, and recently from 2013-2015 \citep{Cloern15}.

Orthophosphate (PO$_4^{3-}$) and \ac{din} enter the Delta primarily through the Sacramento and San Joaquin rivers and from municipal \ac{wwtp} inputs.  Annual nutrient export from the Delta region has been estimated as approximately 30 thousand kg d$^{-1}$ of total nitrogen \citep[varying with flow,][]{Novick15}, with 90\% of ammonium (NH$_4^{+}$) originating solely from the Sacramento Regional County Treatment Facility (hereafter Sacramento \ac{wwtp}) \citep{Jassby08}.  Although nitrogen and phosphorus inputs are considerable, primary production is relatively low and not nutrient-limited \citep{Jassby02,Kimmerer12}.
...
% n min/max
<<>>=
nobs <- map(mods$data, ~ .x$res %>% na.omit %>% length) %>% 
  unlist
minobs <- min(nobs)
minsta <- mods$Site_Code[which.min(nobs)]
maxobs <- max(nobs)
maxsta <- mods$Site_Code[which.max(nobs)]
@
...
The \ac{wrtds} model is a moving window regression that fits unique parameters (i.e., $\beta_0$, \ldots, $\beta_4$) at each observation point in the time series ($n$ ranging from \Sexpr{minobs} at \Sexpr{minsta} to \Sexpr{maxobs} at \Sexpr{maxsta}).  Rather than fitting a global model to the entire time series, one regression is fit to every observation.  Observations within a window for each regression are weighted relative to annual, seasonal, and flow distances from the observation at the center of the window.  Observations with distances farther from the center (i.e., greater time and different flow values from the center) have less weight during parameter estimation for each regression.  Annual values are described using decimal time as a continuous numeric value (e.g., July 1st, 2017 is 2017.5) and season is described as a numeric value for day of year. Weights within each window are estimated using a tri-cube weighting function, which is similar to a bell curve except the tail ends do not asymptote at zero \citep{Hirsch10}.  This ensures that observations outside of the window are given zero weight, as compared to an infinitely small value.  The final vector of weights used for each regression is the product of the three separate weight vectors for time, season, and flow.  This approach allows for a type of smoothing where the observed fit is specific to the data characteristics within windows. 

Models applied herein were based on a tidal adaptation of the original method that can use either flow or salinity estimates as nutrient predictors \citep{Beck15}.  All models were fit to describe the conditional mean response using a weighted Tobit model for left-censored data below the detection limit \citep{Tobin58}.  Model predictions were evaluated as monthly values that were the same as the day of sampling in the observed data and as annual values that averaged monthly predictions within each water year (October to September). All analyses used the WRTDStidal package for the R statistical programming language \citep{Beck17b,RDCT17}.  The default model fitting procedures were used that set half-window widths as six months for seasonal weights, ten years for annual weights, and half the range of salinity or flow in the input data for $Q$ weights.
...
\subsection{Application of \ac{wrtds} to estuarine systems}

Use of \ac{wrtds} to evaluate long-term trends in water quality has not been extensive in estuarine systems.  To date, \ac{wrtds} has only been applied to evaluate eutrophication trends in Tampa Bay, Florida \citep{Beck15} and the Patuxent River Estuary (Chesapeake Bay, \citealt{Beck17}). Unique lessons were learned from the application to these two systems, including \begin{inparaenum}[1\upshape)]  
...
\end{inparaenum}
The application of \ac{wrtds} to monitoring data from the Delta is the first to apply the method to a large temperate estuary on the Pacific coast.  

New challenges were encountered related to the geographic location and the unique characteristics of \ac{sfe} that furthered our understanding of how the method can be used for trend analysis in tidal waters.  The \ac{sfe} is a macrotidal system that has daily variation in water depth typical of coastal systems on the Pacific Coast (i.e., $>$ 1 m).  Previous applications of \ac{wrtds} in tidal waters were limited to systems with substantially smaller tidal amplitudes (i.e., $<$ 1 m), demonstrating that \ac{wrtds} can be applied in systems with a wide range of tidal influences.  This point is particularly relevant given that hydrologic factors are often key determinants of water quality changes in coastal systems, suggesting the method is robust in the presence of large tidal swings. The \ac{sfe} is also unique in that excess turbidity, rather than nutrients, is the dominant limiting factor of primary production.  This differs substantially from Tampa Bay and Chesapeake Bay, neither of which are light-limited from sedimentation.  Our case study in Suisun Bay provided a unique demonstration of \ac{wrtds} to describe chlorophyll dynamics as a function of multiple factors unrelated to nutrients by evaluating the effects of biological invasion on water quality and the complex interaction of flow on both. This result provided the first application of \ac{wrtds} in tidal waters to evaluate multiple factors that are included explicitly in the model (e.g, flow) and those that are not (e.g., biological forcing).  As such, \ac{wrtds} has the potential to inform the understanding of nutrient and phytoplankton dynamics in coastal systems that vary greatly in physical, chemical, and biological characteristics.    
...
cols <- RColorBrewer::brewer.pal(9, 'Blues')[7] %>% 
  c('grey')
  
ylab <- expression(paste('log - DIN (mg ', L^-1, ')'))
...
p1 <- ggplot(toplo, aes(x = Date)) +
  geom_area(aes(y = log10(1 + val), group = var, fill = var), stat = 'identity', position = 'stack', alpha = 0.75) + 
...
<<prddat, fig.cap = 'Model results (points) and flow-normalized predictions (lines) for ten stations grouped by nitrogen analyte and geographic location in the Delta region (locations in \\cref{fig:delt_map}).  Results are annually-averaged for each water year from October to September.', fig.height = 8, fig.width = 8>>=
data(mods)
data(delt_dat)

cols <-  brewer.pal(9, 'Blues')[c(4:9)] %>% 
  colorRampPalette(.)

stat_meta <- delt_dat %>% 
  dplyr::select(Site_Code, Location) %>% 
  unique %>% 
  mutate(
    Location = factor(Location, 
           levels = c('Delta', 'Middle', 'Suisun'), 
           labels = c('Peripheral', 'Interior', 'Suisun')
    )
  ) %>% 
  arrange(Location, Site_Code) %>% 
  mutate(
    colv = c(cols(4), cols(3), cols(3)), 
    pts = c(1:10), 
    shps = c(c(16, 17, 18, 15), c(16:18), c(16:18))
  )

toplo <- mods %>% 
  mutate(
    anns = map(data, annual_agg), 
    Location = factor(Location, 
                      levels = c('Delta', 'Middle', 'Suisun'), 
                      labels = c('Peripheral', 'Interior', 'Suisun')
                      )
  ) %>% 
  left_join(stat_meta, by = c('Site_Code', 'Location')) %>% 
  select(-data, -flovar) %>% 
  unnest %>% 
  group_by(resvar) %>% 
  nest %>% 
  mutate(
    plts = map(data, function(x){
      
      p <- ggplot(x, aes(x = date, y = bt_fits, group = Site_Code)) +
        geom_point(alpha = 0.9, col = x$colv, pch = x$shps) + 
        geom_line(aes(y = bt_norm), col = x$colv) +
...
suis <- stat_meta %>% 
  filter(Location == 'Suisun')

perileg <- ggplot(peri, aes(x = pts, y = pts, colour = Site_Code, shape = Site_Code)) +
  geom_point() +
  geom_line() +
  theme_minimal() + 
  scale_colour_manual(values = peri$colv, name = 'Peripheral') +
  scale_shape_manual(values = peri$shps, name = 'Peripheral') +
  theme(legend.position = 'top')
perileg <- g_legend(perileg)

inteleg <- ggplot(inte, aes(x = pts, y = pts, colour = Site_Code, shape = Site_Code)) +
  geom_point() +
  geom_line() +
  theme_minimal() + 
  scale_colour_manual(values = inte$colv, name = 'Interior') +
  scale_shape_manual(values = inte$shps, name = 'Interior') +
  theme(legend.position = 'top')
inteleg <- g_legend(inteleg)

suisleg <- ggplot(suis, aes(x = pts, y = pts, colour = Site_Code, shape = Site_Code)) +
  geom_point() +
  geom_line() +
  theme_minimal() + 
  scale_colour_manual(values = suis$colv, name = 'Suisun') +
  scale_shape_manual(values = suis$shps, name = 'Suisun') +
  theme(legend.position = 'top')
suisleg <- g_legend(suisleg)

ylab1 <- expression(paste(NH [4] ^ '+', ' (mg ', L^-1, ')'))
ylab2 <- expression(paste(NO [2] ^ '-', ' / ', NO [3] ^ '2-', ' (mg ', L^-1, ')'))
ylab3 <- expression(paste('DIN (mg ', L^-1, ')'))

pA <- ggplot_gtable(ggplot_build(toplo$plts[[1]]))
pB<- ggplot_gtable(ggplot_build(toplo$plts[[2]]))
pC <- ggplot_gtable(ggplot_build(toplo$plts[[3]]))
maxWidth = unit.pmax(pA$widths[2:3], pB$widths[c(2:3)], pC$widths[c(2:3)])
pA$widths[2:3] <- maxWidth
pB$widths[2:3] <- maxWidth
...
grid.arrange(
  arrangeGrob(
    arrangeGrob(pA, left = textGrob(ylab1, rot = 90)),
    arrangeGrob(pC, left = textGrob(ylab3, rot = 90)), 
    ncol = 1
  ), 
  arrangeGrob(
    arrangeGrob(pB, left = textGrob(ylab2, rot = 90)), 
    arrangeGrob(perileg, inteleg, suisleg, ncol = 1, top = textGrob('\nStations'))
  ), 
  ncol = 2
  )
...
<<stock, fig.height = 3, fig.width = 8, cache = T, out.width = '\\textwidth', fig.cap = 'Nitrogen concentration measurements (mg L$^{-1}$) from the City of Stockton Wastewater Treatment Plant, San Joaquin County.  Wastewater discharge requirements were implemented in 2006 to convert ammonium to nitrate.'>>=
data(stock_conc)
...
locs <- delt_dat %>% 
  select(Location, Site_Code, Latitude, Longitude, nh, no23, din) %>% 
  gather('var', 'val', nh:din) %>% 
  group_by(Location, Site_Code, Latitude, Longitude, var) %>% 
  summarise(
    len = length(na.omit(val)),
    val = median(val, na.rm = TRUE), 
    val = round(val, 2)
  ) %>% 
  ungroup %>% 
  gather('sum', 'val', val:len) %>% 
  unite('varsum', var, sum) %>% 
  spread(varsum, val) %>% 
  mutate(
    Location = factor(Location, 
                      levels = c('Delta', 'Middle', 'Suisun'), 
...
# table stuff
cap.val <- 'Monitoring stations in the upper San Francico Estuary used to evaluate nitrogen trends. Records from 1976 to 2013 were evaluated using the total observations ($n$) at each station. Median values (mg L$^{-1}$) are reported for the entire period of record.'
cgrps <- c('', '', 'DIN', 'NH$_{4}^{+}$', 'NO$_{2}^{-}$/NO$_{3}^{2-}$')
rgrps <- locs$Location %>% unique
stats <- locs$Site_Code
totab <- locs[, -c(1, 2)]
names(totab) <- c('Lat', 'Lon', rep(c('$n$', 'Med.'), 3))
...
tab <- trnds_nrm %>% 
  ...
  mutate(
    med = format(round(med, 1), nsmall = 1), 
    perchg = format(round(perchg, 1), nsmall = 1),
    perchg = gsub('^\\s+', '', perchg),
    perchg = ifelse(grepl('\\-', perchg),
      paste0('(', perchg, ')'),
      paste0('(\\textbf{', perchg, '})')
      ),
    ztest = p_ast(ztest)
...
# table stuff
cap.val <- 'Summaries of flow-normalized trends in nitrogen forms for all stations and annual aggregations. Summaries are  medians (mg L$^{-1}$) and percent change per year in parentheses (increasing in bold). Changes and significance estimates are based on seasonal Kendall tests of flow-normalized results within each time period. *$p<0.05$'
...
# table stuff
cap.val <- 'Summaries of flow-normalized trends in nitrogen forms for all stations and seasonal aggregations from 1976-1995. Summaries are  medians (mg L$^{-1}$) and percent change per year in parentheses (increasing in bold). Changes and significance estimates are based on seasonal Kendall tests of flow-normalized results within each time period. Months for each season are Spring: MAM, Summer: JJA, Fall: SON, Winter: DJF. *$p<0.05$'
vars <- c('DIN', 'NH$_{4}^{+}$', 'NO$_{2}^{-}$/NO$_{3}^{2-}$')
stats <- tab$Site_Code
...
# table stuff
cap.val <- 'Summaries of flow-normalized trends in nitrogen forms for all stations and seasonal aggregations from 1996-2013. Summaries are  medians (mg L$^{-1}$) and percent change per year in parentheses (increasing in bold). Changes and significance estimates are based on seasonal Kendall tests of flow-normalized results within each time period. Months for each season are Spring: MAM, Summer: JJA, Fall: SON, Winter: DJF. *$p<0.05$'
vars <- c('DIN', 'NH$_{4}^{+}$', 'NO$_{2}^{-}$/NO$_{3}^{2-}$')
stats <- tab$Site_Code
