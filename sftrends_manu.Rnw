...
cols <- RColorBrewer::brewer.pal(11, 'RdYlGn')
pal <- colorRampPalette(rev(cols))(1000)
...
toplo <-select(trnds, -data) %>% 
  unnest %>% 
  select(Site_Code, resvar, reslabs, cat, ztest, perchg) %>% 
  left_join(., statmeta, by = 'Site_Code') %>% 
  data.frame %>% 
  mutate(
    rnks = scales::rescale(perchg, c(1, 1000)), 
    rnks = round(rnks), 
    cols = pal[rnks],
    ind = 1:nrow(.), 
    cattype = ifelse(cat %in% molabs, 'mo', 'yr'), 
...
for(uni in unis){
  ...
  for(i in 1:length(bars)){
      ...
      pall <- subview(pall, bars[[i]], x = dat$Longitude, y = dat$Latitude, 0.08, 0.08)
  
  }
    
  pall <- pall + ggtitle(uni)
  plo_ls[[uni]] <- pall
