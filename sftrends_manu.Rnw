...
<<clmchl, fig.height = 9.3, fig.width = 8, out.width = '0.9\\textwidth', cache = T, fig.cap = 'Trends in clam abundance and \\ac{chla} concentration from 1976 to 2013 at station D7 in Suisun Bay.  Invasion by \\textit{Potamocorbula amurensis} clams in the late 1980s and displacement of \\textit{Corbicula fluminea} was shown by changes in clam density (a, annual means), with biomass linked to salinity (b).  A decrease in \\ac{chla} concentration was also observed by changes in annual (c) and seasonal trends (d) based on \\ac{wrtds} results.  Reductions in \\ac{chla} concentration were coincident with an increase in SiO$_2$ concentration (e), with the greatest increase in August (f). A significant ($p < 0.001$) relationship between clam biomass and \\ac{chla} concentration is shown in subfigure (g).  Flow relationships with \\ac{chla} concentration shown by \\ac{wrtds} have also changed over time (h, observations from June).'>>=
...
p5 <- prdnrmplot(sio2d7, predicted = F, col_vec = cols, min_mo = 10) + 
  mythm + 
  scale_x_date('Year') +
  # scale_y_continuous(ylab) +
  labs(title = '(e)')
...
# seasyr plot, sio2
p6 <- seasyrplot(sio2d7, predicted = F, col_vec = cols, min_mo = 10) + 
  theme(
    axis.line.x = element_line(), 
    axis.line.y = element_line(), 
    axis.ticks.x = element_line(),
...
# scatter plot of concentration x biomass with model 
p7 <- ggplot(toplo, aes(x = biomass, y = bt_norm)) + 
  geom_point(size = 3, alpha = 0.8, colour = 'darkgrey') + 
  geom_line(data = data.frame(x = xvals, y = yvals), aes(x, y, colour = 'black'), lwd = 1) +
  mythm + 
...
p8 <- dynaplot(chld7, mo = 6, col_vec = cols, floscl = F, size = 0.5) +
  mythm %+replace%
  theme(
...
pD <- ggplot_gtable(ggplot_build(p7))
maxWidth = unit.pmax(pA$widths[2:3], pB$widths[2:3], pC$widths[c(2:3)], pD$widths[c(2:3)])
...
pE <- ggplot_gtable(ggplot_build(p2))
pF <- ggplot_gtable(ggplot_build(p4))
pG <- ggplot_gtable(ggplot_build(p6))
...
# combine all
grid.arrange(
  arrangeGrob(pA, pB, pC, pD, ncol = 1, heights = c(1, 1, 1, 1)), 
  arrangeGrob(pE, pF, pG, pH, ncol = 1, heights = c(1, 1, 1, 1)), 
  ncol = 2, widths = c(1, 1)
  )
@
