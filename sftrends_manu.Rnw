...
<<obsdat, fig.cap = 'Observed \\ac{din} from ten stations in the upper \\ac{sfe} Delta.  Data were collected monthly and evaluated with \\ac{wrtds} models from 1976 to 2013. Note different y-axis scales.  See \\cref{fig:delt_map} for station locations.', fig.height = 8, fig.width = 6, cache = T, out.width = '0.8\\textwidth'>>=
data(delt_dat)
...
toplo <- delt_dat %>% 
  ...
  mutate(Site_Code = factor(Site_Code, 
    levels = c('D7', 'D6', 'D4', 'D28', 'D26', 'D19', 'MD10', 'P8', 'C10', 'C3'))
    )
...
cols <- RColorBrewer::brewer.pal(9, 'Blues')[c(4, 7)]
...
ggplot(toplo, aes(x = Date)) +
  geom_area(aes(y = val, group = var, fill = var), stat = 'identity', position = 'stack') + 
