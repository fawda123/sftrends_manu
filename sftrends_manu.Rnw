...
<<prddat, fig.cap = 'Model predictions.', fig.height = 8, fig.width = 8>>=
library(tidyverse)
library(grid)
source('R/funcs.R')
...
toplo <- mods %>% 
  ...
  mutate(
    plts = map(data, function(x){
      ...
      p <- ggplot(x, aes(x = date, y = bt_fits, group = Site_Code, colour = Site_Code)) +
        geom_point(alpha = 0.7) + 
        geom_line(aes(y = bt_norm)) +
        ...
        scale_colour_manual(values = cols(10))
...
perileg <- ggplot(peri, aes(x = pts, y = pts, colour = Site_Code)) +
  geom_point() +
  geom_line() +
  theme_minimal() + 
...
inteleg <- ggplot(inte, aes(x = pts, y = pts, colour = Site_Code)) +
  geom_point() +
  geom_line() +
  theme_minimal() + 
...
suisleg <- ggplot(suis, aes(x = pts, y = pts, colour = Site_Code)) +
  geom_point() +
  geom_line() +
  theme_minimal() + 
...
grid.arrange(
  ...
  arrangeGrob(
    ...
    arrangeGrob(perileg, inteleg, suisleg, ncol = 1)
  ), 
  ncol = 2
  )
