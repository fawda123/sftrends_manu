...
cap.val<-'Summaries of flow-normalized trends in chlorophyll ($\\mu$g $L^{-1}$) and silicon dioxide (mg $L^{-1}$) concentrations for different time periods. Summaries are means and percent changes based on annual means within the time periods.  Increasing values are in bold-italics.'
