...
toplo <- select(trnds_seasyr, Site_Code, resvar, ann, cat, norm) %>% 
  ...
  mutate(
    ...
    cattype = fct_collapse(cattype, 
      an = c('1976-1995bef', '1996-2014aft'),
...
grid.arrange(
  leg,
  arrangeGrob(
    arrangeGrob (plo_ls[[1]], plo_ls[[4]], plo_ls[[7]], ncol = 1, top = c('(a) 1976-1995, 1996-2014')), 
    arrangeGrob(plo_ls[[3]], plo_ls[[6]], plo_ls[[9]], ncol = 1, top = c('(b) Sp, Su, Fa, Wi (1976-1995)')),
    arrangeGrob(plo_ls[[2]], plo_ls[[5]], plo_ls[[8]], ncol = 1, top = c('(c) Sp, Su, Fa, Wi (1996-2014)')),
    ncol = 3
    ),
  ncol = 1, 
  heights = c(0.1, 1)
...
# table stuff
cap.val <- 'Summaries of flow-normalized trends in nitrogen analytes for all stations and different time periods.  Summaries are  medians (mg L$^{-1}$) and percent change per year in parentheses (increasing in bold-italic). Changes and significance estimates are based on seasonal Kendall tests of flow-normalized results within each time period. See \\cref{fig:trndmap} for a summary of spatial trends. Months for each season are Spring: MAM, Summer: JJA, Fall: SON, Winter: DJF.'
