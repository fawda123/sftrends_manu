...
ylab2 <- expression(paste(NH [4] ^ '2+', ' (mg ', L^-1, ')'))
...
grid.arrange(
  ...
  ncol = 1,
  heights = c(0.2, 0.8, 0.25, 1, 0.2, 0.25, 2)
  )
@
...
## first set of plots
pdin1 <- prdnrmplot(toplo1$mod[[2]], col_vec = cols[2], logspace = F, min_mo = 10) +
  ptheme + 
  scale_y_continuous(ylab1) +
...
pchl1 <- prdnrmplot(toplo1$mod[[1]], col_vec = cols[2], logspace = F, min_mo = 10) +
  ptheme + 
...
psio1 <- prdnrmplot(toplo1$mod[[3]], col_vec = cols[2], logspace = F, min_mo = 10) +
  ptheme + 
...
## second set of plots
pdin2 <- prdnrmplot(toplo2$mod[[2]], col_vec = cols[2], logspace = F, min_mo = 10) +
  ptheme + 
  scale_y_continuous(ylab1) +
  theme(legend.position = 'none')
...
pchl2 <- prdnrmplot(toplo2$mod[[1]], col_vec = cols[2], logspace = F, min_mo = 10) +
  ptheme + 
  theme(legend.position = 'none') + 
  scale_y_continuous(ylab2)
...
psio2 <- prdnrmplot(toplo2$mod[[3]], col_vec = cols[2], logspace = F, min_mo = 10) +
  ptheme + 
  theme(legend.position = 'none', axis.title.x = element_text(), axis.text.x = element_text()) +
  scale_x_date('Year') +
  scale_y_continuous(ylab3)
...
din <- wrtdstrnd(dinraw, mobrks, yrbrks, molabs, yrlabs, aves = TRUE)
din <- wrtdstrnd(dinraw, mobrks, yrbrks = c(-Inf, Inf), molabs, yrlabs = c('1976-2013'), aves = TRUE) %>% 
  .[1, ] %>% 
  rbind(., din) %>% 
  data.frame(res = 'din', .)
