...
<<trndmap, fig.cap = 'Percent change per year in nitrogen analytes for (a) annual and (b) seasonal aggregations.  Changes are based on seasonal Kendall tests of flow-normalized results within each time period. Station names are shown in the top left panel. Three values less than 5\\% change were truncated for visual clarity of the bar plots, see \\cref{tab:trnds} for actual values. Months for each season are Spring: MAM, Summer: JJA, Fall: SON, Winter: DJF.', fig.height = 8, fig.width = 7, cache = T, out.width = '\\textwidth'>>=
...
# response labels 
reslabs <- c('din an', 'nh an', 'no23 an', 'din mo', 'nh mo', 'no23 mo')
reslevs <- c(
  'DIN', 
...
toplo <- select(trnds_sk, Site_Code, resvar, resvar, cat, ztest, perchg) %>% 
  ...
  mutate(
    ...
    ind = 1:nrow(.), 
    cattype = ifelse(cat %in% molabs, 'mo', 'an')
  )
# plot(perchg ~ ind, dat = toplo, col = cols)
...
for(uni in unis){
  ...
  for(i in 1:length(bars)){
      ...
      pall <- subview(pall, bars[[i]], x = dat$Longitude, y = dat$Latitude, subwid, 0.14)
...
# setup legend
leg <- ggplot(data.frame(x = 1:10, y = 1:10, z = seq(-5, 5, length = 10)), aes(x, y, colour = z)) + 
  geom_point() + 
  scale_colour_gradientn(colours = c(rev(colneg), colpos)) + 
  guides(colour = guide_colourbar(title = expression('% chg yr' ^-1), barheight = 5, barwidth = 0.5))
leg <- g_legend(leg)

grid.arrange(
  arrangeGrob(plo_ls[[1]], plo_ls[[3]], plo_ls[[5]], ncol = 1, top = c('Annual (1976-1995, 1996-2014)')), 
  arrangeGrob(plo_ls[[2]], plo_ls[[4]], plo_ls[[6]], ncol = 1, top = c('Seasonal (Sp, Su, Fa, Wi)')),
  leg,
  ncol = 3, 
  widths = c(1, 1, 0.4)
  )
