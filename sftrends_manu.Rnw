...
<<obsdat, fig.cap = 'Observed \\ac{din} (NH$_4^{+}$ + NO$_2^{-}$/NO$_3^{2-}$) from ten stations in the upper \\ac{sfe} Delta.  Data were collected monthly and evaluated with \\ac{wrtds} models from 1976 to 2013. Note different y-axis scales.  See \\cref{fig:delt_map} for station locations.', fig.height = 7, fig.width = 6, cache = T, out.width = '0.9\\textwidth'>>=
data(delt_dat)
...
toplo <- select(trnds_seasyr, Site_Code, resvar, ann, cat, norm) %>% 
  ...
  filter(!cat %in% c('1976-1995', '1996-2013')) %>% # remove year categories
...
grid.arrange(
  leg,
  arrangeGrob(
    arrangeGrob(plo_ls[[2]], plo_ls[[4]], plo_ls[[6]], ncol = 1, top = c('(a) Sp, Su, Fa, Wi (1976-1995)')),
    arrangeGrob(plo_ls[[1]], plo_ls[[3]], plo_ls[[5]], ncol = 1, top = c('(b) Sp, Su, Fa, Wi (1996-2013)')),
    ncol = 2
    ),
  ncol = 1, 
  heights = c(0.1, 1)
