...
% aliased citations
\defcitealias{Beck15}{Beck and Hagy 2015}
...
Environmental condition is affected my multiple processes acting in a hierarchy across space and time \citep{Oneill89,Levin92}. Trend analysis is a broad discipline that has been applied to evaluate environmental condition using time series for the interpretation of ecologically-relevant changes.  Direct evaluation of an observed time series is often insufficient given that a long-term change can be masked by variation at shorter time scales or the observed variation represents the combined effects of many variables. Climate, local, regional, and historical effects may act individually or together to impose a change on time series, such that methods that account for variation at different scales have been used for trend analysis \citep[\cref{fig:schematic},][]{Bhangu97,Champely97,Chang08,Halliday12}.  As a practical approach for water quality evaluation, trend analysis of eutrophication endpoints often focuses on tracking the change in concentrations or loads of nutrients over many years. Indicators of eutrophication can vary naturally with variation in flow conditions and may also reflect long-term effects of management or policy changes. For example, \ac{chla} concentration as a measure of phytoplankton response to nutrient inputs can follow seasonal patterns with cyclical variation in temperature and light changes throughout each year, whereas annual trends can follow long-term variation in nutrient inputs to the system \citep{Cloern96,Cloern10}. Similarly, nutrient trends that vary with hydrologic loading also vary as a function of utilization rates by primary producers or decomposition processes \citep{Sakamoto89,Schultz08,Harding16}. Time series analysis of water quality indicators must simultaneously consider effects of processes at multiple scales and interactions between variables of interest to develop a more comprehensive description of system change. 

Appropriate methods for the analysis of change depend largely on the question of interest and on charactericstics of the environmental dataset.  Trend analyses for aquatic systems have traditionally focused on comparisons between discrete periods of time to estimate a direction and magnitude of a trend using non-parametric tests \citep{Hirsch91,Esterby96}. Development of these conventional approaches addressed limitations in historical monitoring datasets related to infrequent sampling and relatively few years of continuous data. Increased availability of multi-decadal datasets, particularly for high profile environments, has prompted the development of trend analysis methods that leverage the descriptive potential of long-term time series from continuous monitoring programs \citep{Bowes09,Halliday12}. These methods are often data-driven where the parameterization of a simple functional model can change smoothly over time given that relationships between water quality variables and potential drivers are dynamic.  The \ac{wrtds} approach was developed under this context and has been used to characterize decadal trends in running-water systems \citep{Hirsch10,Sprague11,Medalie12,Hirsch14,Zhang16}. This method has the potential to provide a spatially and temporally robust description of trends by fitting a dynamic model with parameters that change relative to the domain of interest.  More recently, the \ac{wrtds} method was adapted for trend analysis in tidal waters, with a focus on \ac{chla} trends in Tampa Bay \citepalias{Beck15} and the Patuxent River Estuary \citepalias{BeckIP}, and tidally-infleunced time series of dissolved oxygen from continuous sonde measurements \citep{Beck15b}. These studies have demonstrated potential for the use \ac{wrtds} for trend analysis in tidal waters and further application to alternative datasets could provide additional insight into drivers of change in aquatic systems. 

The \ac{sfe} on the Pacific Coast of the United States is one of the most prominent and culturally significant estuaries in the western hemisphere \citep{Cloern15}.  Background nutrient concentrations in the Bay often exceed those associated with excessive primary production, although eutrophication events have historically been infrequent.  Recent changes in response to additional stressors (e.g., variation in freshwater inputs/withdrawals, invasive species, climate change) suggests that Bay condition has not followed past trajectories and more subtle spatial and temporal variation could provide clues that describe underlying properties of this system \citep{Cloern12b}.  The unique ecological and social context of the Bay provides a valuable opportunity to gain insight into ecosystem properties of estuaries that define water quality dynamics at different scales. The Delta region of \ac{sfe} in particular is a mosaic of inflows that receives and processes inputs from the larger watershed to the lower Bay \citep{Jassby00,Jassby02,Jassby08}. A comprehensive monitoring dataset has been collected at several fixed locations in the Delta for the last four decades \citep{Jabusch16}.  Morever, nutrient dynamics in the Delta are inherently linked to flow variation from inputs, withdrawal, impoundments, and downstream transport \citep{Novick15}, suggesting an approach that explicitly considers flow effects is critical for trend analysis.  To date, the Delta monitoring dataset is an under-utitilized data source and a comprehensive analysis with \ac{wrtds} could facilitate an understanding of historical and recent changes in \ac{sfe} water quality.
...
\item quantify and interpret trends over four decades at nine stations in the Delta, including annual, seasonal, and spatial changes in nitrogen analytes and response to flow variation,
\item provide detailed descriptions of three case studies in the context of conceptual relationships modelled with \ac{wrtds}.
\end{inparaenum}
The second objective evaluated three specific water quality stations in the Delta to demonstrate complexities with nutrient response to flow, effects of \ac{wwtp} upgrades on water quality, and effects of biological invasion by benthic filter feeders on primary production. Although quantitative descriptions of change can be ends in themselves, the results were expected to have greater impact as a means to more detailed understanding of ecosystem properties.  Products derived from \ac{wrtds} can be used to inform additional analyses, such as water quality response after removing annual, seasonal, or flow effects. Overall, this analysis is expected to further an ecological and management-based understanding of dynamics in San Francisco Bay, with implications for water quality restoration and protection of this prominent system. 
...
\begin{figure}
\centering
\includegraphics[width=1\textwidth,page=1]{figs/delt_map.pdf}
\caption{The \acl{sfe} Delta and monitoring stations used for analysis. The Delta drains the combined watershed from the Sacramento and San Joaquin rivers (bottom left). All data were obtained from the \acl{iep} website (\url{http://water.ca.gov/bdma/meta/Discrete/data.cfm}, \citealt{IEP13}).}
\label{fig:delt_map}   
\end{figure}

<<trndmap, fig.cap = 'Percent change per year in nitrogen analytes for aggregations by (a) year, (b) seasons from 1976-1995, and (c) seasons from 1996-2013.  Changes are based on seasonal Kendall tests of flow-normalized results within each time period. Station names are shown in the top left panel. Values less than -9\\% change were truncated for visual clarity of the bar plots, see \\cref{tab:trndsann,tab:trndsbef,tab:trndsaft} for actual values. Months for each season are Spring: MAM, Summer: JJA, Fall: SON, Winter: DJF.', fig.height = 8, fig.width = 8, cache = T, out.width = '\\textwidth'>>=
...
# need to shift locations of some stations
statmeta[statmeta$Site_Code %in% 'D26', 'Latitude'] <- 38.12
statmeta[statmeta$Site_Code %in% 'D26', 'Longitude'] <--121.55
statmeta[statmeta$Site_Code %in% 'D7', 'Latitude'] <- 38.13
statmeta[statmeta$Site_Code %in% 'D7', 'Longitude'] <--122.015 
statmeta[statmeta$Site_Code %in% 'C10', 'Latitude'] <-37.68 
statmeta[statmeta$Site_Code %in% 'C10', 'Longitude'] <--121.28 

# response labels 
reslabs <- c('din an', 'nh an', 'no23 an', 'din mobef', 'nh mobef', 'no23 mobef', 'din moaft', 'nh moaft', 'no23 moaft')
reslevs <- rep(c(
  'DIN', 
...
pbase <- ggplot(delt_map, aes(x = long, y = lat)) + 
  geom_polygon(aes(group = group, fill = hole), colour = alpha("lightblue", alph)) +
  theme_bw() + 
  theme(
    ...
    plot.margin = grid::unit(c(3, 3, 3, 3), 'pt'), 
  ...
  scale_fill_manual(values=c(alpha("lightblue", alph), "white"), guide="none")

# bounds on limits
yrng <- c(-9, 9)

# color ramp, red for pos, green for neg
colpos <- RColorBrewer::brewer.pal(9, 'Reds')[3:9]
colneg <- RColorBrewer::brewer.pal(9, 'Greens')[3:9]
palneg <-  colorRampPalette(colneg)(1000)
palpos <-   colorRampPalette(colpos)(1000)

# plot(1:2000, col = c(palneg, palpos), pch = 16, cex = 4)
# points(1000, 1000)

# unnest data frame, assign colors by rank and sign
toplo <- select(trnds_seasyr, Site_Code, resvar, ann, cat, norm) %>% 
  rename(perchg = norm) %>% 
  left_join(., statmeta, by = 'Site_Code') %>% 
  data.frame %>% 
  mutate(
    perchg = pmax(yrng[1], perchg),
    perchg = pmin(yrng[2], perchg),
    negs = sign(perchg),  
    rnks = scales::rescale(abs(perchg), c(1, 1000)), 
    rnks = round(rnks), 
    cols = ifelse(negs == 1, palpos[rnks], palneg[rnks]),
    ind = 1:nrow(.)
    ) %>%
  unite('cattype', cat, ann, sep = '', remove = F) %>%
  mutate(
    cattype = factor(cattype),
    cattype = fct_collapse(cattype, 
      an = c('1976-1995bef', '1996-2013aft'),
      mobef = paste0(c('Spring', 'Summer', 'Fall', 'Winter'), 'bef'), 
      moaft = paste0(c('Spring', 'Summer', 'Fall', 'Winter'), 'aft') 
    )
  )

# unite response varibles and summary period combos
unis <- with(toplo, paste(resvar, cattype)) %>% 
  unique %>% 
  sort

sites <- unique(toplo$Site_Code)

plo_ls <- vector('list', length = length(unis))
names(plo_ls) <- unis

for(uni in unis){
  
  # get variable, agg period
  splits <- strsplit(uni, ' ')[[1]]
  resvart <- splits[1]
  cattypet <- splits[2]
  toplo_tmp <- filter(toplo, resvar %in% resvart & cattype %in% cattypet)
  
  # create individual bar plots as ggplot list
  bars <- setNames(lapply(sites, function(i){
    
    subplo <- filter(toplo_tmp, Site_Code %in% i)
    ggplot(subplo, aes(x=cat, y = perchg, fill = factor(cat))) +
      geom_bar(stat = 'identity', colour = 'black', size = 0.2) + 
      scale_fill_manual(values = as.character(subplo$cols)) + 
      theme_tree() + 
      xlab(NULL) + 
      ylab(NULL) + 
      theme_transparent() +
  ...
  ##
  # add the barplots to the base map
  
  # add site labels if plot is first
  pall <- pbase
  set.seed(5)
  if(which(uni == unis) == 1)
    pall <- pbase + 
  ...
  # keep bar widths the same for cattypes
  subwid <- 0.1
  if(grepl('an$', uni))
    subwid <- 0.06
  ...
  for(i in 1:length(bars)){
      ...
      nms <- names(bars)[i]
      dat <- statmeta[statmeta$Site_Code %in% nms,]
      pall <- subview(pall, bars[[i]], x = dat$Longitude, y = dat$Latitude, subwid, 0.148) # change bar heights here

  }
  
  tval <- reslevs[which(uni == reslabs)]
  pall <- pall + 
    geom_text(data = data.frame(x = lims[1], y = lims[4], tval = as.character(tval)), 
      aes(x = x, y = y, label = tval), parse = TRUE, hjust = 0, vjust = 1)
  plo_ls[[uni]] <- pall
...
# setup legend
rngs <- seq(-9, 7, by = 3)
leg <- ggplot(data.frame(x = seq_along(rngs), y = seq_along(rngs), z = rngs), aes(x, y, colour = z)) + 
  geom_point() + 
  scale_colour_gradientn(colours = c(rev(colneg), colpos)) + 
  guides(colour = guide_colourbar(title = expression('% chg yr' ^-1), barheight = 0.5, barwidth = 8)) + 
...
grid.arrange(
  leg,
  arrangeGrob(
    arrangeGrob (plo_ls[[1]], plo_ls[[4]], plo_ls[[7]], ncol = 1, top = c('(a) 1976-1995, 1996-2013')), 
    arrangeGrob(plo_ls[[3]], plo_ls[[6]], plo_ls[[9]], ncol = 1, top = c('(b) Sp, Su, Fa, Wi (1976-1995)')),
    arrangeGrob(plo_ls[[2]], plo_ls[[5]], plo_ls[[8]], ncol = 1, top = c('(c) Sp, Su, Fa, Wi (1996-2013)')),
    ncol = 3
    ),
  ncol = 1, 
  heights = c(0.1, 1)
...
p1 <- ggplot(toplo2) + 
  geom_area(aes(x = date, y = val, group = var, fill = var), stat = 'identity', position = 'stack') + 
...
<<trndcomp, echo = F, warning = F, message = F, cache = T>>=
data(trnds_seasyr)
...
p1 <- ggplot(toplo1, aes(x = tau, y = Site_Code)) + 
  geom_point(aes(pch = trnd, colour = abs(tau)), size = 4, alpha = 0.8) + 
  ...
  scale_colour_gradient(low = 'grey60', high = 'black', guide = F) + 
...
# summarize din, sio2, chl trends
din <- wrtdstrnd_sk(dinraw, mobrks, yrbrks, molabs, yrlabs)
din <- wrtdstrnd_sk(dinraw, mobrks, yrbrks = c(-Inf, Inf), molabs, yrlabs = c('1976-2013')) %>% 
  .[1, ] %>% 
  rbind(., din) %>% 
  data.frame(res = 'din', .)
chl <- wrtdstrnd_sk(chlraw, mobrks, yrbrks, molabs, yrlabs)
chl <- wrtdstrnd_sk(chlraw, mobrks, yrbrks = c(-Inf, Inf), molabs, yrlabs = c('1976-2013')) %>% 
  .[1, ] %>% 
  rbind(., chl) %>% 
  data.frame(res = 'chl', .)
sio <- wrtdstrnd_sk(sioraw, mobrks, yrbrks, molabs, yrlabs)
sio <- wrtdstrnd_sk(sioraw, mobrks, yrbrks = c(-Inf, Inf), molabs, yrlabs = c('1976-2013')) %>% 
  .[1, ] %>% 
  rbind(., sio) %>% 
  data.frame(res = 'sio', .)
...
totab <- rbind(din, chl, sio) %>% 
  select(res, cat, med, perchg, ztest) %>% 
  mutate(
    med = round(med, 1), 
    perchg = round(perchg, 1),
    perchg = ifelse(
  ...
  select(sep, cat, din_med, din_perchg, chl_med, chl_perchg, sio_med, sio_perchg)
...
names(totab) <- rep(c('Median', '\\% change'), 3)

cap.val<-'Summaries of flow-normalized trends in dissolved inorganic nitrogen (mg L$^{-1}$), chlorophyll ($\\mu$g L$^{-1}$), and silicon dioxide (mg L$^{-1}$) concentrations for different time periods at station D7. Summaries are  medians and percent change per year in parentheses (increasing in bold-italic).  Changes and significance estimates are based on seasonal Kendall tests of flow-normalized results within each time period. Increasing values are in bold-italics. Months for each season are Spring: MAM, Summer: JJA, Fall: SON, Winter: DJF.'
foot.val<-'\\footnotesize *$p<0.05$; **$p<0.005$'
...
cap.val<-'Summaries of flow-normalized trends in dissolved inorganic nitrogen (mg L$^{-1}$), chlorophyll ($\\mu$g L$^{-1}$), and silicon dioxide (mg L$^{-1}$) concentrations for different time periods at station C10. Summaries are  medians and percent change per year in parentheses (increasing in bold-italic).  Changes and significance estimates are based on seasonal Kendall tests of flow-normalized results within each time period. Increasing values are in bold-italics. Months for each season are Spring: MAM, Summer: JJA, Fall: SON, Winter: DJF.'
foot.val<-'\\footnotesize *$p<0.05$; **$p<0.005$'
