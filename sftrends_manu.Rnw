...
A comparison of trends with flow-normalized components relative to observed data is justified because flow and nutrient concentrations were linked at many of the stations in the study area, similar to \cref{fig:dinc10}.  These comparisons are made relative to changes in the magnitude, significance, and direction of the trend, all of which have important implications for decision-making. Overall, the differences in apparent trends underscore the critical importance of considering flow effects in decision-making, where flow is affected by complex relationships between inflow, water withdrawals, and exchange with the larger Bay.     
...
trnds_seasyr <- rbind(trnds_nrm, trnds_fit, trnds_obs) %>% 
  ...
  mutate(Site_Code = factor(Site_Code, levels = c('D7', 'D6', 'D4', 'D28', 'D26', 'D19', 'MD10', 'P8', 'C10', 'C3')))
...
statmeta[statmeta$Site_Code %in% 'D26', 'Latitude'] <- 38.14
...
statmeta[statmeta$Site_Code %in% 'D28', 'Latitude'] <-37.94
statmeta[statmeta$Site_Code %in% 'MD10', 'Latitude'] <-38.1
statmeta[statmeta$Site_Code %in% 'P8', 'Latitude'] <-37.93
statmeta[statmeta$Site_Code %in% 'D19', 'Latitude'] <-38.06
...
# bounds on limits
yrng <- max(abs(range(trnds_seasyr$norm)))
yrng <- c(-1 * yrng, yrng)
...
for(uni in unis){
  ...
  for(i in 1:length(bars)){
      ...
      pall <- subview(pall, bars[[i]], x = dat$Longitude, y = dat$Latitude, subwid, 0.21) # change bar heights here
...
# setup legend
rngs <- seq(yrng[1], yrng[2], length = length(c(colneg, colpos)))
