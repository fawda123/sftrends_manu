...
<<trndmap2, fig.height = 9, fig.width = 6, cache = T, out.width = '0.75\\textwidth', fig.cap = 'test'>>=
...
reslevs <- c(
  'DIN, annual', 
  expression(paste(NH [4] ^ '+', ', annual')),
  expression(paste(NO [2] ^ '-', ' / ', NO [3] ^ '2-', ', annual')),
  'DIN, seasonal', 
  expression(paste(NH [4] ^ '+', ', seasonal')),
  expression(paste(NO [2] ^ '-', ' / ', NO [3] ^ '2-', ', seasonal'))
)
...
pbase <- ggplot(delt_map, aes(x = long, y = lat)) + 
  ...
  scale_fill_manual(values=c("cornflowerblue", "white"), guide="none")
...
toplo <- select(trnds, -data) %>% 
  unnest %>% 
  select(Site_Code, resvar, resvar, cat, ztest, perchg) %>% 
...
for(uni in unis){
  ...
  for(i in 1:length(bars)){
      ...
      pall <- subview(pall, bars[[i]], x = dat$Longitude, y = dat$Latitude, 0.08, 0.1)
  
  }
  
  tval <- reslevs[which(uni == reslabs)]
  pall <- pall + ggtitle(tval)
  plo_ls[[uni]] <- pall
  
}
  
grid.arrange(plo_ls[[1]], plo_ls[[2]], plo_ls[[3]], plo_ls[[4]], plo_ls[[5]], plo_ls[[6]], ncol = 2)
