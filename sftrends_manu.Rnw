...
chldat <- filter(diat_dat, resvar %in% 'chl') %>% 
  ...
  group_by(yr, mo) %>% 
  summarize(res = mean(res, na.rm = TRUE), norm0.5 = mean(norm0.5, na.rm = TRUE)) %>% 
  ungroup
